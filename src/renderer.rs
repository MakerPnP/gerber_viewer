    ...
    pub fn paint_layer(
        ...
        for (index, primitive) in layer.primitives().iter().enumerate() {
            ...
            match primitive {
                ...
                GerberPrimitive::Arc(arc) => arc.render(painter, &view, &transform_matrix, color, shape_number, configuration),
    ...
    fn render(
        ...
        if is_axis_aligned {
            ...
            let screen_corners: Vec<Pos2> = corners
                .iter()
                .map(|corner| {
                    (view.translation + transform_matrix.transform_pos2(screen_center + (*corner).to_vec2()) * view.scale)
                        .to_pos2()
                })
                .collect();
